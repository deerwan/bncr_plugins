...
const jsonSchema = BncrCreateSchema.object({
    qltools: BncrCreateSchema.object({
        url: BncrCreateSchema.string().setTitle("QLTools URL").setDescription("青龙Tools前端地址，例如：http://127.0.0.1:5678").setDefault("http://127.0.0.1:5678"),
    }).setTitle("QLTools配置"),
    simpleGuide: BncrCreateSchema.object({
        appendGuidanceURL: BncrCreateSchema.boolean().setTitle("是否附加教程文章地址").setDescription("是否附加教程文章地址").setDefault(true),
        guidanceURL: BncrCreateSchema.string().setTitle("教程文章地址").setDescription("教程文章地址，wool.envRemark==羊毛名，wool.envName==ck名").setDefault('https://example.site/dir/page/#${wool.envRemark}'),
        simpleGuide: BncrCreateSchema.boolean().setTitle("是否启用附加简化教程").setDescription("是否启用附加简化教程").setDefault(true),
        simpleGuideFilePath: BncrCreateSchema.string().setTitle("简化教程文件地址").setDescription("简化教程文件地址(网络地址(http(s))或本地地址(/)，html或markdown，markdown会自动解析为html)").setDefault('https://example.site/dir/page'),
        articleFrame: BncrCreateSchema.string().setTitle("简化教程所在页面等级").setDescription("简化教程在所在页面的哪个等级（以edge浏览器为例，f12 > 元素 > 选中元素后右键 > 复制 > 复制selector）").setDefault('body > article > div'),
        whereIsWoolName: BncrCreateSchema.string().setTitle("羊毛名称所在等级").setDescription("羊毛名称在Frame内的哪个等级（h1,h2,div,blockquote,p...)").setDefault('h1'),
        whereIsSimpleGuide: BncrCreateSchema.string().setTitle("简化教程内容所在等级").setDescription("简化教程在Frame的哪个等级(只支持查找同级第一个)").setDefault('div > p'),
    }).setTitle("简化教程配置"),
    loglevel: BncrCreateSchema.string().setTitle("日志等级").setDescription("debug,info,warn").setDefault("info"),
...
module.exports = async (s) => {
    ...
    if (!Object.keys(ConfigDB.userConfig).length) {
        return await s.reply('请先发送"修改羊毛配置",或者前往前端web"插件配置"来完成插件首次配置');
    }
    // --main--
    if (s.getFrom() == "cron") {
        ...
        let selection = await s.waitInput(async (s) => {
            ...
            if (await checkInput(s)) {
                ...
                if (woolKeys.includes(msg)) {
                    ...
                    await s.reply('请输入“' + wool.envRemark + '”的CK值' + (ConfigDB.userConfig.simpleGuide.appendGuidanceURL ? '\n抓包教程参看：' + eval('`' + ConfigDB.userConfig.simpleGuide.guidanceURL + '`') : '') + (ConfigDB.userConfig.simpleGuide.simpleGuide ? '\n简要教程：' + await makeSimpleGuide(wool.envRemark) : ''));
    ...
    async function cacheSimpleGuide() { //缓存简化教程
        let filepath = '';
        if (isURL(ConfigDB.userConfig.simpleGuide.simpleGuideFilePath)) {
            const downloadPath = '/bncr/BncrData/public/qltoolsSimpleGuide'
            await axios({
                method: 'get',
                url: ConfigDB.userConfig.simpleGuide.simpleGuideFilePath,
            ...
            filepath = ConfigDB.userConfig.simpleGuide.simpleGuideFilePath;
        }
        let html;
        if (!isHTML(filepath)) {
        ...
        if (html) {
            let pair = [];
            const finder = ConfigDB.userConfig.simpleGuide.whereIsSimpleGuide.split(' > ');
            const htmlLoad = require('cheerio').load(html);
            const found = htmlLoad(ConfigDB.userConfig.simpleGuide.articleFrame + ' ' + ConfigDB.userConfig.simpleGuide.whereIsWoolName);
            found.each((index, element) => {
                const ckName = htmlLoad(element).text();
                //let blk = htmlLoad(element).next(finder[0]);
                let blk;
                if (finder[0] == ConfigDB.userConfig.simpleGuide.whereIsWoolName) {
    ...
    async function cacheWoolList() { //缓存
        let woolList;
        let woolIndex = 1;
        await axios.get(ConfigDB.userConfig.qltools.url + '/v1/api/index/data')
            .then(response => {
    ...
    async function uploadCK(serverId, ckName, ck) { //通过api上传ck
        ...
        const config = {
            method: 'post',
            url: ConfigDB.userConfig.qltools.url + '/v1/api/env/add',
            headers: {
